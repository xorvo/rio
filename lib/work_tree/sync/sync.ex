defmodule WorkTree.Sync do
  @moduledoc """
  Public API for WorkTree sync infrastructure.
  ...
  Set the `WORK_TREE_SYNC_DIR` environment variable to enable sync:

      export WORK_TREE_SYNC_DIR="$HOME/Library/Mobile Documents/com~apple~CloudDocs/WorkTree/sync"
  ...
  ## Usage
      ...
      WorkTree.Sync.sync_now()

      # Track a local change for sync
      WorkTree.Sync.track_change(node_id, "update", %{title: "New title"})

      # Get this device's identity
      WorkTree.Sync.device_info()
  """

  alias WorkTree.Sync.{Worker, ChangeTracker, Device}
  ...
  """
  def sync_dir do
    Application.get_env(:work_tree, :sync_dir)
  end
