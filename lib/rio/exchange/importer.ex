defmodule Rio.Exchange.Importer do
  @moduledoc """
  ...
  import Ecto.Query
  alias Rio.Repo
  alias Rio.MindMaps.{Node, Attachment}
  alias Rio.Events.NodeEvent
  alias Rio.Exchange.{Format, Conflict}
  ...
  defp verify_subtree(node, expected_path, expected_depth, nodes_by_id) do
    ...
    if node.path != actual_path or node.depth != expected_depth do
      path_str = Rio.Ecto.PathType.serialize_path(actual_path)
