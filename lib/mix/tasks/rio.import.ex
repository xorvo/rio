defmodule Mix.Tasks.Rio.Import do
  @moduledoc """
  Imports Rio data from a .wtx exchange file.

  ## Usage

      mix rio.import backup.wtx
  ...
  @shortdoc "Import Rio data from a .wtx file"
  ...
  def run(args) do
    ...
    path =
      case positional do
        [p | _] -> p
        [] ->
          Mix.shell().error("Usage: mix rio.import <path.wtx> [--mode full|merge]")
          exit({:shutdown, 1})
      end
    ...
    case Rio.Exchange.import_file(path, mode: mode) do
      {:ok, stats} ->
