...
Rio is a mind mapping app built with Phoenix LiveView + SQLite. It runs as a web app (`mix phx.server`) or a native macOS desktop app via a menu bar launcher.
...
- `lib/rio/mind_maps/` — core context: CRUD, tree operations, search, layout
- `lib/rio_web/live/mind_map_live/` — main LiveView and handler modules
- `lib/rio/exchange/` — export/import system (`.wtx` format)
...
- **`phoenix-colocated` not found during `mix assets.deploy`**: Run `MIX_ENV=prod RIO_DESKTOP=true mix compile` first to generate colocated hooks in the prod build path.
...
| Mode | Database path |
|------|--------------|
| Dev | `rio_dev.db` (project root) |
| Test | `rio_test.db` (project root) |
| Desktop | `~/Library/Application Support/Rio/rio.db` |
| Prod (web) | `DATABASE_PATH` env var or `~/rio_prod.db` |

Desktop path can be overridden with `RIO_DATA_DIR` env var.

### Data is preserved across app updates

The database lives in `~/Library/Application Support/Rio/`, outside the `.app` bundle. Installing a new version replaces only the application binary.
...
mix rio.import backup.wtx --mode merge
```
...
- Desktop mode detected via `RIO_DESKTOP=true` env var in `config/runtime.exs`

## Release workflow

1. Update version in `native/app-bundle/Info.plist` and `mix.exs`
2. `make desktop-build`
3. `cp -r "Rio.app" /Applications/`
