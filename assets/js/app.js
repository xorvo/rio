  ...
  NodeDrag: {
    ...
    createGhost(e) {
      ...
      if (svgLayer) {
        ...
        edgeGroups.forEach((group) => {
          ...
          for (const node of subtreeNodes) {
            ...
            if (
              Math.abs(sx - (node.x + node.width)) < 20 &&
              sy >= node.y &&
      ...
      for (const node of subtreeNodes) {
        ...
        clonedNode
          .querySelectorAll("[phx-hook]")
          .forEach((el) => el.removeAttribute("phx-hook"));
      ...
      this.ghostOffsetX =
        ((draggedX - minX) * canvasZoom + cursorOffsetInNodeX) * 0.9;
      this.ghostOffsetY =
        ((draggedY - minY) * canvasZoom + cursorOffsetInNodeY) * 0.9;
  ...
  MindMapCanvas: {
    mounted() {
      ...
      this.handleEvent("open-focused-node-link", ({ node_id }) => {
        ...
        console.log(
          "open-focused-node-link event for node:",
          node_id,
          "URL:",
          url
        );
    ...
    handleKeyDown(e) {
      ...
      const isModalOpen =
        document.getElementById("todo-filter-modal") ||
    ...
    startArrowPan() {
      const animate = () => {
        ...
        this.arrowPanSpeed = Math.min(
          this.arrowPanMaxSpeed,
          this.arrowPanSpeed + this.arrowPanAcceleration
        );

        if (this.arrowKeysPressed.has("ArrowLeft"))
          this.panX += this.arrowPanSpeed;
        if (this.arrowKeysPressed.has("ArrowRight"))
          this.panX -= this.arrowPanSpeed;
        if (this.arrowKeysPressed.has("ArrowUp"))
          this.panY += this.arrowPanSpeed;
        if (this.arrowKeysPressed.has("ArrowDown"))
          this.panY -= this.arrowPanSpeed;
